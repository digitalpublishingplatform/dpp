/*!
 * copytoclipboard
 * clipboard copier wrapper using ZeroClipboard or ie capabalities 
 * by luis yoroslav
 * v0.1
 * 03-2013
 */
...
(function ($) {
    var ieCopy = function () { return typeof window.clipboardData != "undefined"; };
    $.initcopytoclipboard = function (element, options, onnoflash, oncopy) {
        var clip;
        if (!ieCopy()) {
            clip = new ZeroClipboard(element, options);            
            clip.on('load', function (client) {
                
            });
            clip.on('noflash', function (client) {
                if (onnoflash && typeof(onnoflash) == "function" ) {
                    onnoflash();
                }                
            });
        ...
        clip.copytoclipboard = function(text) {
            if (ieCopy()) {
                try {
                    ...
                    if (console) {
                        console.log("error trying to copy. message:" + e.message);
                        console.log(e);
                    }
                }
            } else {                
